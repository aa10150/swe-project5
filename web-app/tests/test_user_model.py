...
class TestCreateUser:
    ...
    def test_create_user_success(self, mock_db, sample_user):
        ...
        with patch('api.user_model.db', mock_db):
            result = create_user(sample_user["email"], sample_user["password"], sample_user["name"])
    ...
    def test_create_user_duplicate_email(self, mock_db, sample_user):
        ...
        with patch('api.user_model.db', mock_db):
            # Create first user
            result1 = create_user(sample_user["email"], sample_user["password"], sample_user["name"])
            assert result1 is not None
            
            # Try to create duplicate
            result2 = create_user(sample_user["email"], "different_password", "Different Name")
    ...
    def test_create_user_fields(self, mock_db, sample_user):
        ...
        expected_fields = ["name", "netid", "email", "password", "year", "major", 
                          "interests", "completed_courses", "planned_semesters"]
...
class TestVerifyUser:
    ...
    def test_verify_user_success(self, mock_db, sample_user):
        ...
        with patch('api.user_model.db', mock_db):
            # Create user
            create_user(sample_user["email"], sample_user["password"], sample_user["name"])
...
class TestAddRemoveCompletedCourse:
    ...
    def test_remove_completed_course_success(self, mock_db, sample_user):
        ...
        from api.user_model import create_user, add_completed_course, remove_completed_course
